...
total_combinations = length(slip_ratios) * length(cut_percents);
current_combo = 0;
...
for i = 1:length(slip_ratios)
    for j = 1:length(cut_percents)
        ...
        % Run simulation
        simOut = sim('cbuildv1p0.slx'); % Adjust stop time as needed
...
X = cut_percents;  % TC_CUT as x-axis
Y = slip_ratios;   % kappa_thresh as y-axis
Z = zeros(length(slip_ratios), length(cut_percents));

% =========================================================
% Segment 1 Sweep — Successive Zooming (4 rounds)
% Waypoints at t = 0, 0.333, 0.667, 1
% 4th-order polynomial, 5 coefficients
% Extra constraint: p'(t=0) = 0  =>  a1=0
% p0-p3 all swept
% Optimizing by: distance
% =========================================================
...
for round = 1:n_rounds
    ...
    fprintf('\n========== ROUND %d ==========\n', round);
    fprintf('p0: [%.2f, %.2f]  p1: [%.2f, %.2f]  p2: [%.2f, %.2f]  p3: [%.2f, %.2f]\n', ...
                ...
                for i3 = 1:n
                    ...
                    if max_slip >= 0.5
                        ...
                        results(end+1,:) = [coeff_a0, coeff_a1, coeff_a2, coeff_a3, coeff_a4, ...
                                            p0, p1, p2, p3, v_final, distance, max_slip];
                    end
    ...
    % Find best and zoom — maximize distance (col 11)
    [~, best_idx] = max(results(:, 11));
    best    = results(best_idx, :);
    best_p0 = best(6); best_p1 = best(7);
    best_p2 = best(8); best_p3 = best(9);

    fprintf('Round %d best: p0=%.3f  p1=%.3f  p2=%.3f  p3=%.3f  dist=%.4f  v=%.4f\n', ...
            round, best_p0, best_p1, best_p2, best_p3, best(11), best(10));
    ...
    fprintf('New ranges: p0=[%.2f,%.2f]  p1=[%.2f,%.2f]  p2=[%.2f,%.2f]  p3=[%.2f,%.2f]\n', ...
            p0_range(1), p0_range(2), p1_range(1), p1_range(2), ...
            p2_range(1), p2_range(2), p3_range(1), p3_range(2));
end
...
% Collect top 100 across all rounds — sorted by distance
...
xlsx_path = 'C:\GITHUB\FB26_Controls\Control Algorithms\Formula_LCS_Competition\sweep_seg1_zoomingDistance.xlsx';
headers   = {'a0','a1','a2','a3','a4','p0','p1','p2','p3','v_final','distance','maxSlip'};
...
for i = 1:size(top100, 1)
    fprintf('%-10.4f %-10.4f %-10.4f %-10.4f %-10.4f %-8.2f %-8.2f %-8.2f %-8.2f %-12.4f %-12.4f %-10.4f\n', ...
            top100(i,1), top100(i,2), top100(i,3), top100(i,4), top100(i,5), ...
            top100(i,6), top100(i,7), top100(i,8), top100(i,9), ...
            top100(i,10), top100(i,11), top100(i,12));
end

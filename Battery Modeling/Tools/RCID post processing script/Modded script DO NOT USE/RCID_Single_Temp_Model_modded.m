...
% Author:   Daniel Seals with edits from Mehmet Kara                     %
% Updated:  Febuary 2, 2026                                              %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%% Import Data & Format
% Load Test Data
% Note: Using dir to find Excel files specifically
Formatted_Directory = dir('Formatted RCID Data/*.xlsx'); 

if length(Formatted_Directory) == 1
    % Automatic path
    Test_File_Name = Formatted_Directory(1).name;
    % readtable is used for Excel
    TestData = readtable(fullfile('Formatted RCID Data', Test_File_Name), 'VariableNamingRule', 'preserve');
    
elseif length(Formatted_Directory) > 1 || length(Formatted_Directory) == 0
    disp('Select RCID test file (Excel or CSV).')
    % uigetfile filter updated to show Excel and CSV files
    ...
    TestData = readtable(fullfile(path, file), 'VariableNamingRule', 'preserve');
end
...
V    = TestData{:, 6};
I    = TestData{:, 7};
deltat = 0.1; % Ensure this matches your actual sampling rate (sec)
...
function list = dir_custom(directory_path)
    % Get directory listing
    raw_list = dir(directory_path);
    % Remove directories (including . and ..) and hidden files
    list = raw_list(~[raw_list.isdir] & ~startsWith({raw_list.name}, '.'));
end
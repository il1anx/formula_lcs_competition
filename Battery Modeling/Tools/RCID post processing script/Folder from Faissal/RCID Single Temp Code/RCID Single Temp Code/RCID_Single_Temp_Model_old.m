...
scatter(Time(Final_Pulse_Index),I(Final_Pulse_Index),'b^')
...
for i = 1:length(Pulse_Start_Index)
    Average_Current_Index_All(i) = mean(I(Pulse_Start_Index(i)+2:Pulse_Start_Index(i)+3));
end
...
% After duplicate pulses are remove, re-find & re-index C-rates
for i = 1:length(Pulse_Start_Index)
...
% Plot Identified Current Steps without Duplicate Current Steps
figure('Position',[100 100 1000 600])
movegui('center')
plot(Time,I,'LineWidth',1)
hold on
...
% Plot Identified Voltage Steps without Duplicate Current Steps
figure('Position',[100 100 1000 600])
movegui('center')
plot(Time,V,'LineWidth',1)
hold on
scatter(Time(Pulse_Start_Index),V(Pulse_Start_Index),'go')
scatter(Time(Pulse_Stop_Index),V(Pulse_Stop_Index),'rx')
scatter(Time(Initial_Pulse_Index),V(Initial_Pulse_Index),'b^')
scatter(Time(Final_Pulse_Index),V(Final_Pulse_Index),'k^')
scatter(Time(Cap_Start),V(Cap_Start),'k')
scatter(Time(Cap_End),V(Cap_End),'k')
title('Automatic Detection of Current Pulses of RCID Test')
xlabel('Time [Seconds]')
ylabel('Voltage [V]')
legend('Voltage','Start of Every Pulse','Stop of Every Pulse',...
    'Initial Pulse in Set','Final Pulse in Set','Capacity Test Start',...
    'Capacity Test Stop')
hold off
...
for SOC_test = 1:length(Pulse_Start_Index)
    ...
    OCV(SOC_test) = V(Pulse_Start_Index(SOC_test));
...
if Model_Type == 0 % Zero-Order Model Fault Check & Verification
    for SOC_test = 1:length(Pulse_Start_Index)
        ...
        % Zero-Order Response
        Vest_zero = OCV(SOC_test) - R_full(SOC_test).*I_new ;
        
        if(nonzeros(V_new>Datasheet.V_max-0.001 | V_new<Datasheet.V_min+0.001))
            fault_V(SOC_test) = 1;
            R_full(SOC_test) = NaN;
        else
            fault_V(SOC_test) = 0;
...
elseif Model_Type == 1 % First-Order Model Optimization, Fault Check, and Verification
    for SOC_test = 1:length(Pulse_Start_Index)
        ...
        E0_est(SOC_test) = OCV(SOC_test);                                   % Estimate of E0
...
for ndx1 = 1:length(C_Rate_Index)
    ...
    ECM_Model.C_Rate(ndx1).Temp.OCV = OCV(Pulse_Numbers);
